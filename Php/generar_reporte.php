...
// 1. Capturar datos del filtro
$tipo_filtro = $_POST['tipo_filtro'] ?? 'dia';
$fecha_inicio = $_POST['fecha_inicio'] ?? date("Y-m-d");
$fecha_fin = $_POST['fecha_fin'] ?? '';
$accion = $_POST['accion'] ?? 'ver';

// 2. Construir la condición SQL dinámicamente
...
// 3. Consulta Maestra
$sql = "SELECT m.ID_MUE, m.FEC_MUE, m.HOR_MUE, m.ORI_MUE, m.RES_MUE, 
...
// 4. Manejo de Exportación a Excel
if ($accion == "excel") {
...
<head>
    ...
    <style>
        ...
        th { background-color: #6b1d1d; color: white; padding: 10px; font-size: 13px; text-align: center; }
        td { border: 1px solid #ddd; padding: 8px; text-align: center; font-size: 12px; }
        tr:nth-child(even) { background-color: #f9f9f9; }
        .no-print { display: inline-block; margin-bottom: 20px; text-decoration: none; color: #6b1d1d; font-weight: bold; border: 1px solid #6b1d1d; padding: 5px 10px; border-radius: 4px; }
...
<body>
    ...
    <?php if($accion == "ver"): ?>
        <a href="../Html/reportes.html" class="no-print">← Volver al Panel de Reportes</a>
    <?php endif; ?>
    ...
    <table>
        ...
        <tbody>
            ...
            if ($resultado && $resultado->num_rows > 0) {
                ...
                echo "<tr><td colspan='14'>No se encontraron registros para el criterio seleccionado.</td></tr>";
            }
            ?>
        </tbody>
    </table>
